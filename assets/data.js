window.AI_EPIC_DATA = {
  "generatedAt": "2026-05-22T06:29:39.410Z",
  ...
  "articles": [
    ...
    },
    {
      ...
      "summary": "AiMAMI 是 borawong/AiMaMi 开源仓库中的桌面应用，官方 README 将其描述为“为 OpenAI Codex 打造的桌面伴侣”。它不是 Codex 的 fork 或套壳，而是围绕 Codex 扩展账号、线程、路由、MCP、Skills 和自定义指令等管理能力。 一句话定位 面向 OpenAI Codex 重度用户的跨平台桌面管理工具，...",
      "wordCount": 848,
      "sourceFile": "mydata/我的数据/AI记录/内容/AiMAMI.md",
      "markdown": "# AiMAMI\n\nAiMAMI 是 `borawong/AiMaMi` 开源仓库中的桌面应用，官方 README 将其描述为“为 OpenAI Codex 打造的桌面伴侣”。它不是 Codex 的 fork 或套壳，而是围绕 Codex 扩展账号、线程、路由、MCP、Skills 和自定义指令等管理能力。\n\n## 一句话定位\n\n面向 OpenAI Codex 重度用户的跨平台桌面管理工具，重点解决多账号、会话树、模型路由、MCP/Skills 管理和指令模板管理问题。\n\n## 核心能力\n\n- 多账号管理：添加、切换、删除、登出账号，并监控额度。\n- 自动账号轮换：按额度阈值选择候选账号，必要时提示切换。\n- 线程管理：按项目分组查看 Codex 线程，支持批量操作和丢失线程恢复。\n- 智能路由：在 Codex 模型菜单中注入第三方模型，让官方模型与第三方模型共存。\n- MCP 与 Skills：管理 MCP server 和 skill 包生命周期。\n- 自定义指令：基于模板应用、预览、回滚和追踪指令变更。\n- 跨平台：基于 Tauri 2，支持 macOS 和 Windows。\n\n## 技术栈\n\n官方仓库显示主要技术栈包括：\n\n- Tauri 2\n- React 18\n- TypeScript\n- Vite 6\n- Tailwind CSS 3\n- shadcn/ui\n- TanStack Query\n- Rust\n- i18next / react-i18next\n\n## 适合场景\n\n- 同时使用多个 Codex 账号，需要统一查看额度和切换。\n- Codex 线程很多，需要按项目树状管理和恢复。\n- 希望在 Codex 中使用第三方 OpenAI-compatible、Anthropic、本地模型等 provider。\n- 需要在 GUI 中管理 MCP server、Skills 和自定义指令模板。\n- 希望用桌面应用集中处理 Codex 配置，而不是手动改配置文件。\n\n## 不适合场景\n\n- 只偶尔使用 Codex，不需要多账号或复杂线程管理。\n- 不愿意运行第三方桌面应用接触本地 Codex 配置。\n- 对账号令牌、模型 provider、API key 管理有严格合规要求但未完成安全审查。\n- 期待完全开源所有商业能力。官方 README 提到账号管理、线程管理和智能路由是私有模块，未包含在开源发布中。\n\n## 快速开始\n\n官方 README 给出的开发流程：\n\n```bash\ngit clone https://github.com/borawong/AiMaMi.git\ncd AiMaMi\npnpm install\npnpm tauri dev\n```\n\n构建命令：\n\n```bash\npnpm tauri build\n```\n\n使用前应检查当前 release、系统依赖、许可证和是否需要访问本地 Codex 配置目录。\n\n## 风险和注意事项\n\n- 它会围绕 Codex 账号、线程、模型 provider、MCP 和 Skills 做管理，权限边界需要仔细核对。\n- API key 和账号令牌属于敏感信息，使用前要确认本机密钥存储和备份策略。\n- 路由第三方模型时，需要确认数据会发往哪个 provider。\n- 开源仓库不等于所有功能都开源，应区分公开模块和私有模块。\n- 批量删除线程、清理配置、恢复线程前要先备份并核对路径。\n\n## 与其他工具的关系\n\n- 与 [[AI记录/内容/codex|codex]]：AiMAMI 是围绕 Codex 的桌面增强管理工具。\n- 与 [[AI记录/内容/code-review-graph|code-review-graph]]：AiMAMI 管理 Codex 能力和配置；code-review-graph 提供代码库索引和审查上下文。\n- 与 [[AI记录/内容/obsidian-markdown|obsidian-markdown]] / [[AI记录/内容/obsidian-bases|obsidian-bases]]：无直接功能重叠，只是同属可被 Skills/MCP 管理的工作流生态。\n\n## 使用判断\n\n如果你的 Codex 使用已经进入多账号、多线程、多模型、多 MCP/Skills 的复杂阶段，AiMAMI 值得评估。若只是单账号日常编码，它可能增加额外复杂度。\n\n## 来源\n\n- [borawong/AiMaMi GitHub](https://github.com/borawong/AiMaMi)"
    },
    {
      "id": "note-4",
      "title": "AI工具文档整理规范",
      "fileTitle": "AI工具文档整理规范",
      "categories": [
        "常用SKILLS"
      ],
      "tags": [
      ...
      "summary": "这篇笔记用于统一整理 常用 SKILLS、常用 MCP、常用 Plugin、常用 IDE 和其他 AI 工具记录。 目标不是把官网全文搬进 Obsidian，而是让未来维护者能快速判断：这个工具是什么、什么时候该用、怎么接入、风险在哪里、来源是否可靠。 推荐结构 每篇工具或 Skill 笔记优先使用以下结构： 1. 一句话定位：用一句话说明它解决什么问题。...",
      "wordCount": 869,
      "sourceFile": "mydata/我的数据/AI记录/内容/AI工具文档整理规范.md",
      "markdown": "# AI工具文档整理规范\n\n这篇笔记用于统一整理 [[AI记录/目录/常用SKILLS|常用 SKILLS]]、[[AI记录/目录/常用MCP|常用 MCP]]、[[AI记录/目录/常用Plugin|常用 Plugin]]、[[AI记录/目录/常用IDE|常用 IDE]] 和其他 AI 工具记录。\n\n目标不是把官网全文搬进 Obsidian，而是让未来维护者能快速判断：这个工具是什么、什么时候该用、怎么接入、风险在哪里、来源是否可靠。\n\n## 推荐结构\n\n每篇工具或 Skill 笔记优先使用以下结构：\n\n1. `一句话定位`：用一句话说明它解决什么问题。\n2. `定位`：类型、运行环境、目标用户、核心能力。\n3. `什么时候用` 或 `适合场景`：列出真实任务场景。\n4. `不适合场景`：说明不要误用的边界。\n5. `安装和接入`：只写当前核验过的入口；命令变化快时提醒以官方文档为准。\n6. `典型工作流`：从开始到验证结果的步骤。\n7. `常用命令`、`工具` 或 `提示词写法`：只列高频用法。\n8. `与其他工具的关系`：说明和相近工具如何分工。\n9. `风险和注意事项`：权限、隐私、写文件、联网、锁文件、过期索引等。\n10. `使用判断`：给出是否应该启用的决策规则。\n11. `来源`：官方文档、GitHub、包管理器、市场页、社区资料。\n\n## Frontmatter\n\n推荐字段：\n\n```yaml\n---\ntitle: 工具名\ntags:\n  - AI记录/MCP\n  - AI记录/已核对\nsource:\n  - https://example.com/official-doc\n  - https://github.com/owner/repo\nlast_checked: 2026-05-20\nstatus: 已核对\n---\n```\n\n`status` 建议使用：\n\n- `已核对`：已确认官方/GitHub/包管理器来源，基本用法可信。\n- `待核对`：名称、来源、维护状态或工具边界仍不确定。\n- `偏浅`：已有简介，但缺少使用方法、来源或风险边界。\n- `需更新`：来源明确，但内容明显落后于当前版本。\n\n## 来源优先级\n\n优先级从高到低：\n\n1. 官方文档。\n2. 官方 GitHub 仓库 README、release、docs。\n3. 包管理器页面，例如 npm、PyPI、Cargo、Homebrew。\n4. 官方市场页或插件目录。\n5. 作者博客、官方社交账号。\n6. 社区目录、MCP 聚合站、教程文章。\n7. Reddit、论坛、二手总结。\n\n社区资料可以用来发现线索，但不应作为高风险安装、权限配置、数据边界的唯一依据。\n\n## 写法要求\n\n- 不复制大段官方文档，保留链接并做中文摘要。\n- 命令要有上下文，例如运行前提、作用、是否会写文件。\n- 若一个名称对应多个候选工具，先写“名称歧义”，不要强行归类。\n- 若工具会读取本地代码、设计稿、记忆、浏览器登录态，应明确隐私边界。\n- 若工具会修改文件、安装插件、配置 MCP 或启动后台服务，应明确“会产生什么持久状态”。\n- 对快速变化的产品，`last_checked` 只能表示“这一天核对过”，不能表示长期有效。\n\n## 目录维护\n\n目录页不应只是一串链接。建议给每个条目附简短状态：\n\n- `已核对`：可作为常用推荐。\n- `待核对`：保留观察，不作为明确推荐。\n- `偏浅`：有笔记但还不够指导使用。\n- `需更新`：来源已过期或命令疑似变化。\n\n示例：\n\n```markdown\n1. [[AI记录/内容/context7|context7]] - 已核对；外部库文档检索。\n2. [[AI记录/内容/code-review-graph|code-review-graph]] - 已核对；大型仓库审查和影响分析。\n3. [[AI记录/内容/buddycode|buddycode]] - 待核对；名称存在多个候选。\n```\n\n## 验证清单\n\n- [ ] Frontmatter 能被 Obsidian 正常识别。\n- [ ] 内部链接使用 wikilink，外部链接使用 Markdown URL。\n- [ ] 至少有一个可信来源。\n- [ ] 有“什么时候用”和“不适合场景”。\n- [ ] 涉及安装或配置时写明副作用。\n- [ ] 涉及本地文件、账号、浏览器、设计稿或记忆时写明隐私边界。\n- [ ] 结论与 `status` 一致。"
    },
    {
      "id": "note-5",
      "title": "Antigravity",
    ...
    },
    {
      ...
      "summary": "buddycode 这个名称仍存在明显歧义。当前公开搜索中至少有两个方向相近但性质不同的候选：一个更像 AI coding desktop/tool，另一个更像编程教育平台。 在未确认具体指向前，不应把它写成已确认的常用 AI 编程 IDE。 一句话定位 这是一个待核对条目；现在的重点不是学习用法，而是确认“你记录的 buddycode 到底是哪一个产品”。...",
      "wordCount": 541,
      "sourceFile": "mydata/我的数据/AI记录/内容/buddycode.md",
      "markdown": "# buddycode\n\n`buddycode` 这个名称仍存在明显歧义。当前公开搜索中至少有两个方向相近但性质不同的候选：一个更像 AI coding desktop/tool，另一个更像编程教育平台。\n\n在未确认具体指向前，不应把它写成已确认的常用 AI 编程 IDE。\n\n## 一句话定位\n\n这是一个待核对条目；现在的重点不是学习用法，而是确认“你记录的 buddycode 到底是哪一个产品”。\n\n## 候选 1：BuddyCode GPT\n\n`buddycodegpt.com` 方向更接近 AI 编程工具。公开页面信息显示它与 LM Studio、Ollama 这类本地模型运行器有关，可能主打本地模型辅助 coding。\n\n需要继续确认：\n\n- 是否仍在维护。\n- 是否有 GitHub 仓库或官方文档。\n- 是否支持打开本地仓库。\n- 是否支持读取和修改文件。\n- 是否能运行测试、终端命令或生成 diff。\n- 是否能和 VS Code、JetBrains、CLI 或 Git 集成。\n- 数据是否完全本地处理，还是会上传到外部服务。\n\n## 候选 2：Buddy Code Hub\n\n`buddycodehub.com` 更像 coding、robotics、electronics、digital literacy 教育平台。这个方向不等同于 AI 编程 IDE，通常不应放在 `常用IDE`。\n\n需要继续确认：\n\n- 是否与你记录的 `buddycode` 是同一个工具。\n- 是否有 AI coding agent 能力。\n- 是否只是教育课程或学习平台。\n- 是否有面向本地代码库的开发工作流。\n\n## 评估任务\n\n如果后续要确认它是否值得纳入常用 IDE，可以用以下任务评估：\n\n1. 打开一个已有仓库并解释结构。\n2. 修改一个小 bug。\n3. 为一个函数补测试。\n4. 运行测试或给出可执行验证方式。\n5. 展示生成 diff 或文件改动的方式。\n6. 说明模型来源和隐私边界。\n\n## 结论模板\n\n```markdown\n- 确认名称：\n- 官方地址：\n- GitHub/文档：\n- 类型：\n- 是否 AI 编程工具：\n- 是否支持本地仓库：\n- 是否支持文件修改：\n- 是否支持终端/测试：\n- 模型来源：\n- 隐私边界：\n- 是否保留在常用 IDE：\n```\n\n## 当前结论\n\n保持 `待核对`。如果确认是 BuddyCode GPT 或类似 AI coding 工具，再扩写为常用 IDE 文档；如果确认是 Buddy Code Hub，则应从 [[AI记录/目录/常用IDE|常用 IDE]] 移出，改放到教育工具或其他候选记录。\n\n## 来源\n\n- [BuddyCode GPT 候选](https://buddycodegpt.com/)\n- [Buddy Code Hub 候选](https://www.buddycodehub.com/)"
    },
    {
      "id": "note-12",
    ...
    },
    {
      ...
      "summary": "claude mem 是 thedotmack/claude mem 维护的 Claude Code 持久记忆插件。它通过 Claude Code lifecycle hooks 捕获会话中的工具使用观察，把观察压缩成结构化记忆，再在后续会话中按需注入相关上下文。 一句话定位 当你希望 Claude Code 跨会话记住项目经验、历史 bug、常用命令和协作...",
      "wordCount": 1200,
      "sourceFile": "mydata/我的数据/AI记录/内容/claude mem.md",
      "markdown": "# claude mem\n\n`claude-mem` 是 `thedotmack/claude-mem` 维护的 Claude Code 持久记忆插件。它通过 Claude Code lifecycle hooks 捕获会话中的工具使用观察，把观察压缩成结构化记忆，再在后续会话中按需注入相关上下文。\n\n## 一句话定位\n\n当你希望 Claude Code 跨会话记住项目经验、历史 bug、常用命令和协作偏好时，用 `claude-mem` 做持久记忆层。\n\n## 定位\n\n- 类型：Claude Code plugin + hooks + worker service + MCP/search tools。\n- 主要对象：Claude Code 用户，也支持 Gemini CLI、OpenCode、OpenClaw 等集成路径。\n- 数据形态：会话、观察、摘要、搜索索引。\n- 本地服务：官方 README 提到 worker/web viewer 默认使用 `http://localhost:37777`。\n- 存储：官方文档说明使用 SQLite，并有向量/全文搜索组件。\n\n## 什么时候用\n\n- 长期维护同一个代码库，希望保留跨会话经验。\n- 项目有固定测试命令、构建流程、发布流程。\n- 需要让 Agent 记住“之前为什么这么改”。\n- 希望通过搜索历史观察减少重复排查。\n- 团队有可沉淀的非敏感协作约定。\n\n## 不适合场景\n\n- 临时一次性任务。\n- 高敏感代码库或数据边界不清的环境。\n- 不能接受本地 hooks、后台 worker、数据库和插件缓存的机器。\n- Windows 或特殊 shell 环境中没有验证插件稳定性时，不应直接作为强依赖。\n- 需要事实正确性时，不能把记忆当作权威来源；记忆可能过期或被错误压缩。\n\n## 安装和接入\n\n官方 README 给出两条主要安装路径：\n\n```text\nnpx claude-mem install\n```\n\n或在 Claude Code 内通过插件 marketplace：\n\n```text\n/plugin marketplace add thedotmack/claude-mem\n/plugin install claude-mem\n```\n\n安装后需要重启 Claude Code。官方说明也强调：`npm install -g claude-mem` 只安装 SDK/library，不会注册插件 hooks 或 worker service。\n\nClaude Code 官方插件文档说明，第三方插件通过 marketplace 安装前要确认信任来源，因为插件可能包含 hooks、MCP servers 或其他本地执行能力。\n\n## 核心组件\n\n- lifecycle hooks：在会话开始、用户提交提示词、工具调用后、停止、会话结束等时机收集上下文。\n- worker service：负责 API、web viewer、搜索等后台能力。\n- SQLite database：保存 sessions、observations、summaries。\n- `mem-search` skill：用自然语言查询项目历史。\n- Chroma/vector search：用于语义检索和关键词检索结合。\n\n## 典型工作流\n\n1. 在可信项目环境中安装插件。\n2. 重启 Claude Code，让 hooks 生效。\n3. 正常进行开发、调试、审查会话。\n4. 插件自动捕获工具使用观察并生成摘要。\n5. 后续会话启动时，插件注入相关上下文。\n6. 需要主动查历史时，用 memory search 工具先搜索引，再按 ID 取详细观察。\n\n## 搜索使用方法\n\n官方 README 描述了三层检索模式：\n\n1. `search`：先查紧凑索引，按 query、type、date、project 过滤。\n2. `timeline`：围绕某个观察或查询查看时间线。\n3. `get_observations`：只对筛出的 ID 拉取完整细节，并尽量批量取。\n\n使用原则是先过滤，再拉全文，避免把大量历史记忆直接塞进上下文。\n\n## 适合保存的信息\n\n- 项目结构说明。\n- 常用测试命令。\n- 编码规范。\n- 分支和发布流程。\n- 历史 bug 的排查结论。\n- 非敏感的个人偏好。\n\n## 不应保存的信息\n\n- API key、token、密码。\n- 私人身份信息。\n- 客户数据、商业敏感数据。\n- 未确认的猜测。\n- 可以直接从代码或官方文档获得的过期副本。\n\n## 风险和注意事项\n\n- 这是第三方插件，不是 Anthropic 官方插件；安装前要审查仓库、版本和 issue。\n- hooks 会观察工具使用过程，可能捕获命令输出和文件路径。\n- 记忆压缩可能丢失细节或形成错误概括。\n- 本地 worker、数据库、插件缓存会产生持久状态。\n- 对高风险任务，必须回到当前代码、测试和官方文档验证。\n- 官方文档和社区反馈显示该插件迭代很快，升级后应重新确认稳定性。\n\n## 与其他记忆方式的关系\n\n- 与 `CLAUDE.md`：`CLAUDE.md` 适合显式、稳定、人工维护的项目规则；`claude-mem` 适合自动沉淀会话经验。\n- 与普通聊天上下文：聊天上下文会随会话结束丢失；`claude-mem` 目标是跨会话检索。\n- 与 [[AI记录/内容/context7|context7]]：Context7 查外部库文档；`claude-mem` 查本项目历史记忆。\n\n## 使用判断\n\n如果项目是长期维护、重复上下文很多、机器环境可信，可以试用。若项目包含敏感数据，或者你不能接受第三方 hooks 和本地后台服务，应保持禁用。\n\n## 来源\n\n- [claude-mem GitHub](https://github.com/thedotmack/claude-mem)\n- [claude-mem 官方文档](https://docs.claude-mem.ai)\n- [Claude Code Discover Plugins 官方文档](https://code.claude.com/docs/en/discover-plugins)"
    },
    {
      "id": "note-17",
      ...
      "summary": "code review graph code review graph 当前按 tirth8205/code review graph 与 PyPI 包记录。它是本地代码知识图谱 MCP server，用 Tree sitter 解析仓库结构，把函数、类、导入、调用、测试覆盖等信息建成图，供 AI code review 和日常 coding 任务按影响范围...",
      "wordCount": 1131,
      "sourceFile": "mydata/我的数据/AI记录/内容/code-review-graph.md",
      "markdown": "# code-review-graph\n\n`code-review-graph` 当前按 `tirth8205/code-review-graph` 与 PyPI 包记录。它是本地代码知识图谱 MCP server，用 Tree-sitter 解析仓库结构，把函数、类、导入、调用、测试覆盖等信息建成图，供 AI code review 和日常 coding 任务按影响范围读取上下文。\n\n## 一句话定位\n\n当代码库变大、diff 影响范围不明显、AI 审查容易读太多无关文件时，用 `code-review-graph` 建本地知识图谱。\n\n## 定位\n\n- 类型：本地代码知识图谱 + MCP server + CLI。\n- 目标：减少 AI 审查和代码理解时的无效上下文读取。\n- 核心技术：Tree-sitter AST、SQLite graph、增量更新、blast-radius analysis。\n- 支持对象：Codex、Claude Code、Cursor、Windsurf、Zed、Continue、OpenCode、Antigravity、Gemini CLI、GitHub Copilot 等。\n- 本地存储：官方 README 说明图谱使用 `.code-review-graph/` 下的 SQLite 文件，无外部数据库或云依赖。\n\n## 什么时候用\n\n- 大型仓库 review。\n- PR/diff 影响范围分析。\n- 想知道某个变更可能影响哪些函数、类、调用链和测试。\n- 希望 AI review 先看图谱和风险项，而不是扫全仓。\n- 需要架构热点、hub、bridge、社区结构、知识缺口分析。\n- 希望为新成员或 Agent 生成代码库 wiki。\n\n## 不适合场景\n\n- 很小的单文件改动。官方 benchmark 也说明 trivial edit 中图谱上下文可能比直接读文件更重。\n- 不支持或解析质量较差的语言/框架模式。\n- 需要精确运行时行为分析的动态路径。\n- 代替测试、类型检查或人工审查。\n- 项目不允许生成本地索引目录时。\n\n## 安装和接入\n\n官方 PyPI/GitHub 当前给出的 quick start：\n\n```text\npip install code-review-graph\ncode-review-graph install\ncode-review-graph build\n```\n\n也可以使用：\n\n```text\npipx install code-review-graph\n```\n\n安装命令会自动检测支持的平台并写入对应 MCP 配置。也可以指定平台：\n\n```text\ncode-review-graph install --platform codex\ncode-review-graph install --platform claude-code\ncode-review-graph install --platform cursor\n```\n\n官方要求 Python 3.10+。如果有 `uv`，MCP 配置会优先使用 `uvx`；否则使用 `code-review-graph` 命令。\n\n## 典型工作流\n\n1. 在代码仓库根目录安装并配置平台。\n2. 运行 `code-review-graph build` 初次解析代码库。\n3. 在 AI 客户端中请求构建或更新图谱。\n4. 修改代码后使用增量更新或自动 hook 更新图谱。\n5. review diff 时先调用变更检测、影响半径和 review context 工具。\n6. 只读取图谱指出的相关文件、调用链和测试缺口。\n7. 仍然运行测试和静态检查确认结果。\n\n## 常用 CLI\n\n```text\ncode-review-graph install\ncode-review-graph install --platform <name>\ncode-review-graph build\ncode-review-graph update\ncode-review-graph status\ncode-review-graph watch\ncode-review-graph visualize\ncode-review-graph wiki\ncode-review-graph detect-changes\n```\n\n## 常用 MCP 工具方向\n\n当前本地 Codex 环境可见的工具包括：\n\n- `build_or_update_graph_tool`：构建或增量更新图谱。\n- `get_minimal_context_tool`：获取极简上下文入口。\n- `detect_changes_tool`：分析 diff、风险和审查建议。\n- `get_review_context_tool`：生成聚焦审查上下文。\n- `get_impact_radius_tool`：分析变更影响半径。\n- `semantic_search_nodes_tool`：搜索函数、类、文件等节点。\n- `query_graph_tool`：查 caller、callee、importer、tests 等关系。\n- `list_flows_tool` / `get_flow_tool`：查看执行流。\n- `list_communities_tool` / `get_community_tool`：查看代码社区。\n- `refactor_tool`：重命名预览、死代码和重构建议。\n\n实际工具名以当前 MCP server 暴露内容为准。\n\n## 支持语言\n\n官方 README/PyPI 描述当前支持 24 种语言及 notebooks，包括 Python、TypeScript/TSX、JavaScript、Vue、Svelte、Go、Rust、Java、Scala、C#、Ruby、Kotlin、Swift、PHP、Solidity、C/C++、Dart、R、Perl、Lua、Zig、PowerShell、Julia、Nix，以及 Jupyter/Databricks `.ipynb`。\n\n## 风险和限制\n\n- 图谱可能过期，review 前要更新。\n- 动态语言和框架魔法可能导致调用图不完整。\n- 影响分析偏保守，可能出现 false positives。\n- 官方 benchmark 提到 flow detection 在部分语言/框架上召回有限。\n- 小型单文件变更可能不值得引入图谱上下文。\n- 生成 `.code-review-graph/` 索引目录前要确认项目允许本地持久文件。\n- 它不能替代测试、类型检查、lint 和人工判断。\n\n## 与其他工具的关系\n\n- 与 [[AI记录/内容/serena|serena]]：Serena 更偏符号级定位和编辑；`code-review-graph` 更偏审查上下文、影响范围和结构图。\n- 与 [[AI记录/内容/deepwiki|deepwiki]]：DeepWiki 查 GitHub 仓库文档化结构；`code-review-graph` 查本地当前工作树。\n- 与 [[AI记录/内容/context7|context7]]：Context7 查外部库 API；`code-review-graph` 查本地代码影响。\n- 与 [[AI记录/内容/codex|codex]] / [[AI记录/内容/claude code|claude code]]：可作为它们进行 code review 时的上下文工具。\n\n## 使用判断\n\n当仓库较大、改动跨模块、review token 成本高或影响范围不明显时，优先考虑。普通小改动、脚本文件或一次性任务可以直接读文件和运行测试。\n\n## 来源\n\n- [code-review-graph GitHub](https://github.com/tirth8205/code-review-graph)\n- [code-review-graph PyPI](https://pypi.org/project/code-review-graph/)"
    },
    {
      "id": "note-18",
      "title": "codex 命令大全",
    ...
    },
    {
      ...
      "summary": "[!note] 来源说明 本笔记基于 OpenAI 2026 04 27 官方文章《An open source spec for Codex orchestration: Symphony》以及 openai/symphony GitHub 仓库整理。Symphony 当前更接近开源规范和参考实现，不是一个面向普通用户的一体化 IDE 产品。 一句话结论...",
      "wordCount": 2226,
      "sourceFile": "mydata/我的数据/AI记录/内容/OpenAI Symphony.md",
      "markdown": "# OpenAI Symphony\n\n> [!note] 来源说明\n> 本笔记基于 OpenAI 2026-04-27 官方文章《An open-source spec for Codex orchestration: Symphony》以及 `openai/symphony` GitHub 仓库整理。Symphony 当前更接近开源规范和参考实现，不是一个面向普通用户的一体化 IDE 产品。\n\n## 一句话结论\n\nSymphony 是 OpenAI 发布的 **coding agent 编排规范与参考实现**。它把 Linear 这样的项目管理看板变成 coding agents 的控制面：每个可执行 issue 对应一个隔离 workspace，后台服务持续拉取任务、启动 Codex agent、监控状态、重试失败任务，并把结果交给人审查。\n\n可以把它理解为：\n\n```text\nIssue Tracker / Linear\n        ↓\nSymphony\n        ↓\nPer-issue workspace + Codex app-server session\n        ↓\nPR / 评论 / CI / 人工审查 / 任务状态流转\n```\n\n它不是模型，也不是 IDE。它的价值在于把「人盯着多个 Codex 会话」改成「人管理任务队列和审查结果」。\n\n## 它解决的问题\n\n交互式 coding agent 的瓶颈不是单个任务能不能做，而是人类注意力。工程师同时开几个 Codex 会话、分配任务、看输出、补充提示、处理卡住的流程，很快会被上下文切换拖垮。\n\nSymphony 的切入点是把工作抽象成 issue，而不是抽象成会话或 PR：\n\n- issue 是目标单位，agent session 和 PR 是实现手段。\n- Linear 看板承担状态机角色，例如 Todo、In Progress、Human Review、Merging、Done。\n- 每个 active issue 都可以被调度成一个独立 agent run。\n- agent 卡住、崩溃或正常退出后，orchestrator 可以按规则继续、重试或停止。\n- 人主要审查结果和调整系统规则，而不是逐步遥控 agent。\n\nOpenAI 官方文章提到，在部分团队中，采用 Symphony 后前三周 landed PR 数量增加了 500%。这个数字应理解为特定团队和流程下的结果，不等同于所有团队可复现的通用提升。\n\n## 和 Agent Harness 的关系\n\nSymphony 可以视为一种面向 coding agent 的 **execution harness + orchestrator**。\n\n它具备 harness 的几个典型职责：\n\n- 准备隔离 workspace。\n- 把 issue、workflow、prompt 和工具上下文交给 agent。\n- 限制 agent 在对应 workspace 内执行。\n- 管理运行状态、失败、重试、超时和停止。\n- 提供结构化日志和可观测性。\n- 把团队规则沉淀到仓库内的 `WORKFLOW.md`。\n\n同时它又超出单会话 harness，进入多任务编排层：\n\n- 轮询 issue tracker。\n- 控制并发数量。\n- 根据 blocker、priority、状态排序调度。\n- 在多个 issue 之间分配 agent。\n- 对终态 issue 清理 workspace。\n- 把 Linear 这样的外部系统作为长期状态来源。\n\n因此更准确的定位是：**Symphony 是把 harness 工程推进到项目管理层的一种实现范式**。\n\n## 它是不是自带 IDE\n\n不是。\n\nSymphony 不提供传统意义上的编辑器，也不替代 VS Code、Cursor、JetBrains 或 Obsidian。它也不是一个「打开后直接写代码」的应用。\n\n它需要一个实际执行代码任务的 agent runtime。OpenAI 参考实现使用的是 **Codex app-server mode**：\n\n```text\nSymphony 负责任务编排\nCodex app-server 负责执行 coding agent 会话\nGit / CI / Linear / gh CLI 等工具负责具体工程动作\nIDE 仍然用于人工接管、审查或手动修改\n```\n\n理论上，其他 coding agent 也可以实现同样的 runtime 适配层。例如有人可以按 `SPEC.md` 写一个面向 Claude Code、Gemini CLI 或自研 agent 的 Symphony 实现。但 OpenAI 仓库中的参考实现默认围绕 Codex app-server。\n\n## 核心工作流\n\n典型流程如下：\n\n1. 团队在 Linear 中创建或维护 issue。\n2. Symphony 按固定间隔轮询 active states。\n3. Orchestrator 判断 issue 是否可调度，例如是否被 blocker 阻塞、是否还有并发空位。\n4. Workspace Manager 为 issue 创建稳定、隔离的 workspace。\n5. 运行 `WORKFLOW.md` 中定义的 hooks，例如 clone repo、安装依赖、准备环境。\n6. Agent Runner 启动 Codex app-server session。\n7. Symphony 将 issue 信息和 `WORKFLOW.md` prompt 模板渲染成 agent 任务。\n8. Codex 在 workspace 内工作，可能提交 PR、更新 Linear、回应 review、等待 CI。\n9. Symphony 根据 issue 状态、agent 退出、超时、错误决定继续、重试、停止或清理。\n10. 人类在 Linear、GitHub PR、CI 和 dashboard 中审查最终结果。\n\n这套流程的关键不是「一次 prompt 做完所有事」，而是让任务变成可重复、可观察、可恢复的后台执行单元。\n\n## 仓库结构\n\n`openai/symphony` 仓库当前主要由三部分组成：\n\n- `README.md`：项目定位、运行方式、预览性质说明。\n- `SPEC.md`：语言无关的 Symphony Service Specification，是最核心的部分。\n- `elixir/`：OpenAI 提供的 Elixir/OTP 实验参考实现。\n\n仓库 README 明确给出两种使用方式：\n\n- 让自己喜欢的 coding agent 按 `SPEC.md` 实现一个 Symphony。\n- 使用 `elixir/` 目录里的实验参考实现。\n\n这说明 OpenAI 更强调规范，而不是绑定一个唯一产品形态。\n\n## 关键组件\n\n按 `SPEC.md` 的抽象，Symphony 至少包含这些组件：\n\n- `Workflow Loader`：读取 `WORKFLOW.md`，解析 YAML front matter 和 Markdown prompt。\n- `Config Layer`：把配置转换为类型化运行参数，处理默认值、环境变量和路径。\n- `Issue Tracker Client`：读取 Linear issue，规范化状态、标签、blocker、优先级等字段。\n- `Orchestrator`：拥有调度循环、并发控制、运行状态、重试队列和 reconciliation。\n- `Workspace Manager`：为 issue 创建、复用、清理 workspace，并执行 hooks。\n- `Agent Runner`：启动 coding agent app-server，把 prompt 发给 agent，并回传事件。\n- `Status Surface`：可选的人类可读状态面，例如终端或 web dashboard。\n- `Logging`：结构化日志，用于调试多 agent 并发运行。\n\n## `WORKFLOW.md` 的角色\n\n`WORKFLOW.md` 是 Symphony 的仓库内契约。它通常包含两部分：\n\n- YAML front matter：配置 tracker、polling、workspace、hooks、agent、codex 等运行参数。\n- Markdown body：真正发给 Codex 的 prompt 模板。\n\n这很关键，因为团队可以把 agent 的行为规则和运行约束跟代码一起版本化：\n\n- 哪些 issue 状态算 active。\n- workspace 放在哪里。\n- 新 workspace 如何初始化。\n- Codex 用什么命令启动。\n- 每次 agent run 前后执行哪些 hook。\n- agent 应该如何处理实现、测试、PR、review 和任务状态。\n\n这也是 Symphony 与普通脚本的差异：它不是一次性自动化脚本，而是把长期 agent 工作流写成可演进的仓库契约。\n\n## 适合的场景\n\nSymphony 更适合这些工作：\n\n- 例行实现任务。\n- 多 issue 的迁移、升级、清理。\n- CI 修复、rebase、冲突处理、review feedback 处理。\n- 可以通过 ticket 描述清楚目标和验收标准的任务。\n- 需要大量并行探索，但允许人类筛选结果的任务。\n- 大型 monorepo 中「最后一公里」较慢的 PR 落地流程。\n\n不太适合的场景：\n\n- 需求极度模糊，必须持续与人沟通澄清。\n- 涉及高风险生产操作，且没有足够 sandbox、审批和回滚机制。\n- 缺少测试、缺少文档、缺少可重复验证命令的仓库。\n- 团队尚未把 issue、PR、CI、review 状态标准化。\n\n## 使用前提\n\nSymphony 对仓库和团队流程有要求。它更适合已经做过 harness engineering 的团队：\n\n- 仓库有清晰的测试命令和 CI。\n- 任务可以通过 issue 写清楚。\n- agent 有足够文档知道如何构建、测试、提交和验证。\n- 高风险命令有审批或沙箱策略。\n- Linear / GitHub / CI 的状态流转相对稳定。\n- 团队愿意把失败反馈转化为 guardrails、skills、docs 和 workflow 改进。\n\n如果这些基础缺失，Symphony 只会把混乱并行化。\n\n## 与 Codex CLI、Claude Code 的关系\n\n| 名称 | 在 Symphony 语境中的角色 |\n|---|---|\n| Symphony | 编排器和运行规范 |\n| Codex app-server | OpenAI 参考实现默认调用的 agent runtime |\n| Codex CLI | 可以提供 app-server 能力的 Codex 工具入口 |\n| Claude Code | 非官方默认，但理论上可以被适配成执行层 |\n| Linear | 默认 issue tracker 和状态机 |\n| GitHub / gh CLI / CI | agent 交付和验证工作的工程系统 |\n| VS Code / Cursor | 人工接管和审查时使用的 IDE |\n\n最重要的边界是：Symphony 负责调度和环境，agent runtime 负责真正读代码、改代码、跑命令。\n\n## 风险与限制\n\nOpenAI 仓库 README 将 Symphony 标注为低调的工程预览，适合可信环境测试。它不是成熟托管产品，也不是一套默认安全的企业平台。\n\n需要重点关注：\n\n- `SPEC.md` 不强制统一的 sandbox 或审批策略。\n- agent 命令会在 workspace 内运行，workspace 边界检查必须可靠。\n- hooks 可能执行 shell 脚本，必须避免路径、引号、删除命令等误操作。\n- Linear 状态机设计会直接影响 agent 是否启动、停止或清理。\n- 多 agent 并发会放大仓库中测试不稳定、文档缺失和权限配置错误的问题。\n- 如果让 agent 写 ticket、改状态、合并 PR，需要额外定义审计和权限边界。\n\n对个人或小团队来说，先用它理解「任务级 agent 编排」的架构价值，比直接在生产仓库里开高并发更现实。\n\n## 推荐阅读顺序\n\n1. [[AI记录/内容/Agent Harness：从能聊天到能干活的智能体运行框架|Agent Harness：从能聊天到能干活的智能体运行框架]]\n2. [[AI记录/内容/OpenAI Symphony|OpenAI Symphony]]\n3. [[AI记录/内容/OpenAI Symphony：SPEC 范式|OpenAI Symphony：SPEC 范式]]\n4. [[AI记录/内容/OpenAI Symphony：Elixir 参考实现|OpenAI Symphony：Elixir 参考实现]]"
    },
    {
      "id": "note-37",
      "title": "OpenAI Symphony：Elixir 参考实现",
      "fileTitle": "OpenAI Symphony：Elixir 参考实现",
      "categories": [
        "AI研究所"
      ],
      "tags": [
      ...
      "summary": "定位 elixir/ 是 OpenAI 在 openai/symphony 仓库中提供的实验性参考实现。它基于仓库根目录的 SPEC.md，用于展示如何把 Symphony 规范落到真实可运行的长驻服务。 OpenAI 在 README 中明确提醒：Symphony Elixir 是 evaluation only prototype，建议团队基于 SPEC...",
      "wordCount": 1508,
      "sourceFile": "mydata/我的数据/AI记录/内容/OpenAI Symphony：Elixir 参考实现.md",
      "markdown": "# OpenAI Symphony：Elixir 参考实现\n\n## 定位\n\n`elixir/` 是 OpenAI 在 `openai/symphony` 仓库中提供的实验性参考实现。它基于仓库根目录的 `SPEC.md`，用于展示如何把 Symphony 规范落到真实可运行的长驻服务。\n\nOpenAI 在 README 中明确提醒：Symphony Elixir 是 evaluation-only prototype，建议团队基于 `SPEC.md` 实现自己的 hardened version。\n\n因此它适合：\n\n- 学习 Symphony 的真实运行形态。\n- 参考模块划分和行为边界。\n- 在可信环境中试验。\n- 让 coding agent 帮助搭建本地原型。\n\n不应直接把它当作无需审计的生产级平台。\n\n## 为什么是 Elixir\n\nOpenAI 给出的理由是 Elixir/OTP 适合长时间运行的监督进程：\n\n- BEAM/OTP 擅长管理长驻服务和子进程。\n- Supervisor 模型适合 agent worker 的启动、失败和恢复。\n- Elixir 生态有成熟工具链。\n- 开发时支持较好的热更新体验。\n\nSymphony 的核心问题不是单次脚本执行，而是持续调度、并发 worker、失败恢复、状态观察。Elixir/OTP 和这个问题形态比较匹配。\n\n## 它如何工作\n\nElixir 参考实现的高层流程：\n\n1. 轮询 Linear 中的候选工作。\n2. 为每个 issue 创建 workspace。\n3. 在 workspace 内启动 Codex app-server mode。\n4. 把 workflow prompt 发给 Codex。\n5. 让 Codex 持续处理 issue，直到工作完成或状态变化。\n\n运行期间，Symphony 还会提供一个 client-side `linear_graphql` 工具，让 repo skills 可以通过 Symphony 的 Linear 权限执行原始 GraphQL 调用，例如评论、状态更新或上传相关操作。\n\n如果已 claimed 的 issue 进入终态，例如 `Done`、`Closed`、`Cancelled`、`Duplicate`，Symphony 会停止该 issue 的 active agent，并清理匹配 workspace。\n\n## 依赖关系\n\n主要依赖：\n\n- Linear API token。\n- Codex app-server。\n- Elixir/Erlang 运行环境。\n- `mise`，官方推荐用于管理 Elixir/Erlang 版本。\n- Git 或其他 workspace 初始化工具。\n- 可选 Phoenix observability service。\n\n运行参考实现前，目标仓库最好已经适合 agent 工作：测试命令清晰、CI 可用、文档完整、任务和状态流转稳定。\n\n## 基本使用流程\n\n官方 README 给出的思路是：\n\n1. 确认代码库适合 agent 工作。\n2. 在 Linear 中创建 personal API key，并设置为 `LINEAR_API_KEY`。\n3. 将 `elixir/WORKFLOW.md` 复制到自己的仓库。\n4. 可选复制 `commit`、`push`、`pull`、`land`、`linear` 等 repo-local skills。\n5. 修改 `WORKFLOW.md`，适配自己的 Linear project slug、状态机、workspace 和 hooks。\n6. 安装运行时依赖并启动服务。\n\n官方示例命令大致是：\n\n```bash\ngit clone https://github.com/openai/symphony\ncd symphony/elixir\nmise trust\nmise install\nmise exec -- mix setup\nmise exec -- mix build\nmise exec -- ./bin/symphony ./WORKFLOW.md\n```\n\n也可以传入自定义 workflow 文件：\n\n```bash\n./bin/symphony /path/to/custom/WORKFLOW.md\n```\n\n如果不传路径，默认使用当前目录的 `./WORKFLOW.md`。\n\n## `WORKFLOW.md` 配置重点\n\nElixir README 中的最小示例包含：\n\n```md\n---\ntracker:\n  kind: linear\n  project_slug: \"...\"\nworkspace:\n  root: ~/code/workspaces\nhooks:\n  after_create: |\n    git clone git@github.com:your-org/your-repo.git .\nagent:\n  max_concurrent_agents: 10\n  max_turns: 20\ncodex:\n  command: codex app-server\n---\n\nYou are working on a Linear issue {{ issue.identifier }}.\n\nTitle: {{ issue.title }} Body: {{ issue.description }}\n```\n\n几个关键点：\n\n- 缺失配置会使用默认值。\n- `tracker.api_key` 未设置时会读 `LINEAR_API_KEY`。\n- `workspace.root` 支持 `~` 和 `$VAR`。\n- `codex.command` 保留为 shell command string。\n- Markdown body 为空时，会使用包含 issue identifier、title、body 的默认 prompt。\n- `hooks.after_create` 常用于 clone repo 和初始化依赖。\n\n## Codex 安全默认值\n\nElixir 参考实现 README 提到，当 policy 字段省略时，会使用较安全的 Codex 默认值：\n\n- `codex.approval_policy` 默认拒绝 sandbox approval、rules、MCP elicitation 等需要额外授权的请求。\n- `codex.thread_sandbox` 默认是 `workspace-write`。\n- `codex.turn_sandbox_policy` 默认 root 在当前 issue workspace。\n\n这不代表整体系统默认适合生产。它只是说明参考实现尽量把 Codex 执行限制在 issue workspace 内。hooks、Linear 权限、GitHub 权限、CI 权限仍需要团队自己审计。\n\n## 可选 dashboard\n\nElixir 参考实现可以通过 `--port` 启动 Phoenix observability service。\n\n包含：\n\n- `/`：LiveView dashboard。\n- `/api/v1/state`：JSON 状态。\n- `/api/v1/<issue_identifier>`：单 issue 详情。\n- `/api/v1/refresh`：刷新入口。\n\n这层属于可观测性，不是 Symphony 的核心调度逻辑。没有 dashboard，调度原则仍然成立。\n\n## 项目结构\n\n`elixir/` 目录中比较重要的内容：\n\n- `README.md`：参考实现说明。\n- `WORKFLOW.md`：本地运行的 workflow 契约示例。\n- `WORKFLOW.md` 相关配置和 prompt：决定 Codex 如何接任务。\n- `lib/`：应用代码和 Mix tasks。\n- `test/`：ExUnit 覆盖。\n- `docs/`：实现相关文档。\n- `Makefile`：构建和测试入口。\n- `mix.exs`：Elixir 项目配置。\n\n根目录的 `.codex/` 还包含仓库本地的 Codex skills 和 setup helpers。\n\n## 测试方式\n\n普通测试：\n\n```bash\nmake all\n```\n\n真实外部端到端测试需要 Linear 凭据，并会创建临时 Linear 资源、启动真实 Codex app-server session：\n\n```bash\ncd elixir\nexport LINEAR_API_KEY=...\nmake e2e\n```\n\n`make e2e` 当前覆盖本地 worker 和 SSH worker 两类场景。若未设置 `SYMPHONY_LIVE_SSH_WORKER_HOSTS`，SSH 场景会用 docker compose 启动临时 SSH workers。\n\n这类测试会接触真实外部服务，不适合在不了解影响的情况下随便运行。\n\n## SSH Worker 扩展\n\n`SPEC.md` 的 Appendix A 描述了 SSH worker extension。Elixir README 的 e2e 也覆盖 SSH worker 场景。\n\n这个扩展的思想是：\n\n- 中央 orchestrator 仍然负责调度和状态。\n- worker run 可以分配到远程 SSH host。\n- workspace root 在远程 host 上解释。\n- Codex app-server 通过 SSH stdio 启动。\n- 每个 host 可以有自己的并发上限。\n\n风险也更高：\n\n- 远程环境可能漂移。\n- workspace 可能是 host-local，迁移 host 意味着冷启动。\n- 路径和 shell quoting 更敏感。\n- host 故障不能导致重复执行同一个 ticket。\n- 观测信息必须标明哪个 host 拥有 run。\n\n## 什么时候参考 Elixir 实现\n\n适合参考：\n\n- 你想理解 Symphony 从 SPEC 到服务的映射。\n- 你需要一个可运行样例来观察 issue、workspace、Codex session 的联动。\n- 你准备实现自己的 Symphony，想先看 OpenAI 怎么切模块。\n- 你想验证 `WORKFLOW.md`、Linear 状态机和 Codex app-server 的配合方式。\n\n不适合直接照搬：\n\n- 生产环境没有权限边界设计。\n- 没有审计 hooks。\n- 没有隔离 workspace root。\n- 没有明确人类审批策略。\n- 团队未准备好处理 agent 自动产生的 PR、评论、状态变更。\n\n## 与 SPEC 的关系\n\n可以把关系理解为：\n\n```text\nSPEC.md\n  = 行为契约和架构标准\n\nelixir/\n  = OpenAI 的实验性落地样例\n```\n\n学习 Symphony 时，应先理解 SPEC，再看 Elixir。否则容易误以为 Symphony 就是一个 Elixir 项目，而忽略它真正想表达的是任务级 agent 编排范式。"
    },
    {
      "id": "note-38",
      "title": "OpenAI Symphony：SPEC 范式",
      "fileTitle": "OpenAI Symphony：SPEC 范式",
      "categories": [
        "AI研究所"
      ],
      "tags": [
      ...
      "summary": "定位 SPEC.md 是 Symphony 最重要的部分。它不是某个语言框架的 README，而是一份 语言无关的 coding agent orchestration service 规范 。 它定义的问题是： 这份规范的价值在于：不同团队可以用 Elixir、TypeScript、Go、Python 或其他语言实现 Symphony，只要满足同一套行为契...",
      "wordCount": 2019,
      "sourceFile": "mydata/我的数据/AI记录/内容/OpenAI Symphony：SPEC 范式.md",
      "markdown": "# OpenAI Symphony：SPEC 范式\n\n## 定位\n\n`SPEC.md` 是 Symphony 最重要的部分。它不是某个语言框架的 README，而是一份 **语言无关的 coding agent orchestration service 规范**。\n\n它定义的问题是：\n\n```text\n持续读取项目管理系统中的工作项\n为每个可执行工作项创建隔离 workspace\n在 workspace 中运行 coding agent session\n用统一状态、重试、日志和 workflow 契约管理这些 session\n```\n\n这份规范的价值在于：不同团队可以用 Elixir、TypeScript、Go、Python 或其他语言实现 Symphony，只要满足同一套行为契约。\n\n## 核心边界\n\n`SPEC.md` 对 Symphony 的边界划得很清楚：\n\n- Symphony 是 scheduler、runner 和 tracker reader。\n- Symphony 不直接规定 ticket 如何写评论、如何改状态、如何贴 PR 链接。\n- ticket 写入通常由 coding agent 通过工具完成。\n- 成功运行不一定意味着 issue 进入 `Done`，也可能只是进入 `Human Review` 之类的交接状态。\n- 规范不强制一个统一 sandbox、审批或 operator confirmation 策略。\n\n这意味着 Symphony 更像「任务级 agent runtime 规范」，而不是固定业务系统。\n\n## 目标\n\nSPEC 的目标可以概括为：\n\n- 固定节奏轮询 issue tracker。\n- 有界并发地派发任务。\n- 维护单一权威 orchestrator 状态。\n- 为每个 issue 创建确定性的 workspace。\n- issue 状态变化后能停止不再适合继续运行的 agent。\n- 对暂时性失败做指数退避重试。\n- 从仓库内 `WORKFLOW.md` 加载运行行为。\n- 至少提供结构化日志这样的可观测性。\n- 支持重启恢复，不要求必须依赖数据库恢复完整内存状态。\n\n## 非目标\n\nSPEC 明确不追求：\n\n- 丰富 web UI 或多租户控制面。\n- 通用工作流引擎。\n- 分布式任务调度平台。\n- 内置 PR、评论、ticket 状态更新业务逻辑。\n- 强制所有实现使用同一种 sandbox 或审批策略。\n\n这能避免把 Symphony 做成过重的平台。它关注的是 coding agent 编排的最小核心。\n\n## 分层模型\n\nSymphony 可以按六层理解：\n\n| 层级 | 责任 |\n|---|---|\n| Policy Layer | 仓库定义的 `WORKFLOW.md` prompt 和团队规则 |\n| Configuration Layer | 解析 front matter，处理默认值、环境变量、路径 |\n| Coordination Layer | 调度循环、并发、eligibility、retry、reconciliation |\n| Execution Layer | workspace 生命周期、hook、agent subprocess |\n| Integration Layer | Linear API 适配和 issue 规范化 |\n| Observability Layer | 日志、状态面、运行指标 |\n\n这套分层是移植 Symphony 的关键。实现语言可以变，但这些责任最好不要混在一起。\n\n## 主组件\n\n### Workflow Loader\n\n读取 `WORKFLOW.md`，解析 YAML front matter 和 Markdown body。\n\n输出结构通常可以理解为：\n\n```text\n{ config, prompt_template }\n```\n\n其中 `config` 负责运行参数，`prompt_template` 是每个 issue 发给 agent 的任务模板。\n\n### Config Layer\n\nConfig Layer 提供类型化读取：\n\n- polling interval。\n- workspace root。\n- active states 和 terminal states。\n- max concurrent agents。\n- Codex command。\n- hook timeout。\n- retry backoff。\n\n它还负责：\n\n- 默认值。\n- `$VAR_NAME` 环境变量解析。\n- `~` 路径展开。\n- 相对路径基于 `WORKFLOW.md` 所在目录解析。\n\n### Issue Tracker Client\n\n当前 SPEC 版本以 Linear 为 tracker。它需要：\n\n- 拉取 active states 中的 candidate issues。\n- 根据 issue IDs 刷新当前状态。\n- 拉取 terminal-state issues 用于启动清理。\n- 把 Linear payload 规范化为稳定 issue model。\n\n规范化后的 issue 包含：\n\n- `id`\n- `identifier`\n- `title`\n- `description`\n- `priority`\n- `state`\n- `branch_name`\n- `url`\n- `labels`\n- `blocked_by`\n- `created_at`\n- `updated_at`\n\n### Orchestrator\n\nOrchestrator 是单一权威状态拥有者。它负责：\n\n- poll tick。\n- running issue map。\n- claimed issue set。\n- retry queue。\n- completed bookkeeping。\n- Codex token 和 runtime 聚合。\n- rate limit 快照。\n\n核心原则是：不要让每个 worker 各自决定全局调度。全局调度必须集中，才能避免重复执行、并发超限和状态竞争。\n\n### Workspace Manager\n\nWorkspace Manager 将 issue identifier 映射为稳定 workspace path。\n\n重要规则：\n\n- issue identifier 用于人类可读日志和 workspace 命名。\n- workspace key 必须 sanitize，只允许安全字符。\n- agent 启动前必须确认 workspace path 没有逃出 workspace root。\n- terminal issue 需要清理对应 workspace。\n\n这层是安全边界之一。任何实现都不能把 workspace 路径当成普通字符串随便拼。\n\n### Agent Runner\n\nAgent Runner 负责：\n\n- 创建或复用 workspace。\n- 执行 `before_run` hook。\n- 启动 coding agent app-server。\n- 渲染 prompt。\n- 发起 turn。\n- 将 agent event 回传 orchestrator。\n- run 结束后执行 `after_run`。\n\nOpenAI 参考实现使用 Codex app-server，但 SPEC 的思想可以移植到其他 agent runtime。\n\n## `WORKFLOW.md` 范式\n\n`WORKFLOW.md` 是 Symphony 的核心用户接口。它让团队把「agent 应该怎样工作」写进仓库。\n\n典型结构：\n\n```md\n---\ntracker:\n  kind: linear\n  project_slug: \"...\"\nworkspace:\n  root: ~/code/workspaces\nhooks:\n  after_create: |\n    git clone git@github.com:your-org/your-repo.git .\nagent:\n  max_concurrent_agents: 10\n  max_turns: 20\ncodex:\n  command: codex app-server\n---\n\nYou are working on a Linear issue {{ issue.identifier }}.\n\nTitle: {{ issue.title }}\nBody: {{ issue.description }}\n```\n\nfront matter 负责配置，body 负责 prompt。这个拆分非常重要：运行时策略和 agent 任务说明都可以版本化。\n\n## 配置领域\n\n### `tracker`\n\n定义任务来源。当前规范支持 `linear`。\n\n关键字段：\n\n- `kind`\n- `endpoint`\n- `api_key`\n- `project_slug`\n- `active_states`\n- `terminal_states`\n\n### `polling`\n\n定义轮询节奏：\n\n- `interval_ms` 默认 30000。\n\n### `workspace`\n\n定义 workspace root：\n\n- 默认为系统临时目录下的 `symphony_workspaces`。\n- 支持 `~`。\n- 支持 `$VAR`。\n- 相对路径基于 `WORKFLOW.md` 所在目录解析。\n\n### `hooks`\n\n定义 workspace 生命周期脚本：\n\n- `after_create`：workspace 首次创建后执行。\n- `before_run`：每次 agent attempt 前执行。\n- `after_run`：每次 agent attempt 后执行，失败只记录日志。\n- `before_remove`：删除 workspace 前执行，失败只记录日志。\n- `timeout_ms`：hook 超时，默认 60000。\n\n对 Windows/PowerShell 环境要特别谨慎：如果 hook 里涉及删除、移动、路径拼接、引号转义，应优先使用明确的 shell 和安全路径检查。\n\n### `agent`\n\n定义编排层的 agent 策略：\n\n- `max_concurrent_agents`\n- `max_turns`\n- `max_retry_backoff_ms`\n- `max_concurrent_agents_by_state`\n\n这些配置决定并发、连续 turn 数、退避上限和按状态限流。\n\n### `codex`\n\n定义 Codex app-server 相关参数：\n\n- `command`\n- `approval_policy`\n- `thread_sandbox`\n- `turn_sandbox_policy`\n- `turn_timeout_ms`\n- `read_timeout_ms`\n- `stall_timeout_ms`\n\nSPEC 建议实现把 Codex 自有配置当作 pass-through，而不是在 Symphony 里手写一份易过期枚举。\n\n## 调度逻辑\n\n一次 poll tick 大致是：\n\n1. reconcile 已运行 issue。\n2. 校验当前 workflow/config。\n3. 拉取 candidate issues。\n4. 按优先级和创建时间排序。\n5. 检查并发空位。\n6. 对 eligible issue dispatch。\n7. 更新状态面和日志。\n8. 安排下一次 tick。\n\neligibility 通常受这些因素影响：\n\n- issue 是否在 active state。\n- 是否已经 running、claimed 或 retrying。\n- blocker 是否全部结束。\n- 当前全局并发是否有空位。\n- 当前状态的并发限制是否有空位。\n\n## Reconciliation\n\nReconciliation 负责让后台运行与 tracker 状态保持一致。\n\n如果 running issue 被刷新后发现：\n\n- 仍在 active state：更新 running entry 中的 issue 信息。\n- 进入 terminal state：停止 agent，并清理 workspace。\n- 进入非 active 且非 terminal state：停止 agent，但不清理 workspace。\n\n这使 Linear 不只是输入队列，也是运行中的控制面。\n\n## Retry 和 continuation\n\nSPEC 区分异常退出和正常退出：\n\n- 异常退出：进入指数退避 retry。\n- 正常退出：如果 issue 仍处于 active state，会安排短 continuation retry，让 agent 有机会继续处理后续状态。\n\n这点很符合 coding agent 的真实工作流：一次 turn 或一次 session 结束，不代表 issue 的所有工作已经完成。\n\n## 可观测性\n\n最低要求是结构化日志。日志应包含：\n\n- issue id。\n- issue identifier。\n- session id。\n- 运行状态。\n- 错误原因。\n- token 使用。\n- rate limit 信息。\n\n可选状态面可以是终端、web dashboard 或 JSON API。但 SPEC 不把 rich UI 作为核心要求。\n\n## 安全重点\n\n实现 Symphony 时，最需要关注的不是 UI，而是边界：\n\n- workspace root containment。\n- issue identifier sanitize。\n- hook 执行 shell 的路径和引号。\n- agent launch cwd 必须是对应 workspace。\n- 删除 workspace 前必须确认目标在 workspace root 内。\n- approval 和 sandbox 策略必须文档化。\n- ticket 写入和 PR 合并权限要有审计。\n\nSymphony 放大的是 agent 执行能力，也会放大错误配置的后果。\n\n## 实现检查清单\n\n一个符合 SPEC 的实现至少应覆盖：\n\n- `WORKFLOW.md` 路径选择、解析、动态 reload。\n- 类型化配置和默认值。\n- Linear candidate fetch、state refresh、terminal fetch。\n- per-issue workspace 创建、复用、清理。\n- lifecycle hooks。\n- coding agent app-server subprocess client。\n- prompt 严格渲染。\n- Orchestrator 单一权威状态。\n- 有界并发。\n- retry queue。\n- terminal/non-active 状态停止运行。\n- 结构化日志。\n\n## 如何迁移到非 Codex runtime\n\n如果要适配 Claude Code 或其他 coding agent，需要替换的是 Execution Layer 中的 Agent Runner：\n\n- 启动命令。\n- session 协议。\n- turn 协议。\n- event stream。\n- approval/user-input 处理。\n- usage/rate-limit 遥测。\n- 工具暴露方式。\n\n其余层仍可复用 Symphony 的范式：issue tracker、workspace、orchestrator、workflow、hooks、observability。"
    },
    {
      "id": "note-39",
      "title": "pencil AI 工具",
    ...
    },
    {
      ...
      "summary": "这里的 pencil MCP 当前按 OpenPencil MCP 记录。OpenPencil 是开源 AI native vector design tool，内置 MCP server，可让 Claude Code、Cursor、Windsurf 等 MCP 客户端读取和修改正在运行的 OpenPencil 设计文档。 如果你原本记录的是另一个 Penc...",
      "wordCount": 960,
      "sourceFile": "mydata/我的数据/AI记录/内容/pencil MCP.md",
      "markdown": "# pencil MCP\n\n这里的 `pencil MCP` 当前按 OpenPencil MCP 记录。OpenPencil 是开源 AI-native vector design tool，内置 MCP server，可让 Claude Code、Cursor、Windsurf 等 MCP 客户端读取和修改正在运行的 OpenPencil 设计文档。\n\n如果你原本记录的是另一个 Pencil/Pencil Spaces/Pencil.dev，需要另建条目或改名，避免混淆。\n\n## 一句话定位\n\nOpenPencil MCP 用来把设计稿接入 AI coding agent，让 Agent 能检查、创建、修改并保存 OpenPencil `.fig` 设计文件。\n\n## 定位\n\n- 类型：设计工具 MCP server。\n- 传输方式：stdio 和 HTTP。\n- 主要对象：Claude Code、Cursor、Windsurf 等支持 MCP 的 AI coding tools。\n- 核心能力：通过运行中的 OpenPencil app 访问设计文档。\n- 工具规模：官方文档列出约 90 个工具，覆盖文档、读取、创建、修改、结构操作和保存。\n\n## 什么时候用\n\n- 让 Agent 读取当前设计稿结构。\n- 让 Agent 按文字要求修改 OpenPencil 画布。\n- 把设计稿作为前端实现参考。\n- 在设计工具和代码实现之间建立可执行工作流。\n- 需要让 AI 创建小型 UI 片段或 hero section 设计草图。\n\n## 不适合场景\n\n- 纯代码库语义检索，这更适合 [[AI记录/内容/serena|serena]] 或 [[AI记录/内容/code-review-graph|code-review-graph]]。\n- 查外部库文档，这更适合 [[AI记录/内容/context7|context7]]。\n- 未打开 OpenPencil app 或未加载设计文档时。\n- 敏感设计文件没有明确文件根目录和权限边界时。\n- 只需要截图视觉参考时，普通浏览器/图片分析可能更简单。\n\n## 安装和 Claude Code 接入\n\n官方文档当前给出的安装方式：\n\n```text\nnpm install -g @open-pencil/mcp\n```\n\nClaude Code 注册方式：\n\n```text\nclaude mcp add --scope user open-pencil -- openpencil-mcp\n```\n\n检查连接：\n\n```text\nclaude mcp list\n```\n\nstdio server 会通过 WebSocket 连接正在运行的 OpenPencil app，默认使用 `7601` 端口。使用前要先打开 OpenPencil desktop app，并加载文档。\n\n## 其他 MCP 客户端配置\n\n其他 MCP 客户端可使用类似配置：\n\n```json\n{\n  \"mcpServers\": {\n    \"open-pencil\": {\n      \"command\": \"openpencil-mcp\"\n    }\n  }\n}\n```\n\n## HTTP 模式\n\nHTTP server 可用于浏览器扩展、脚本、CI 或任意 HTTP client：\n\n```text\nopenpencil-mcp-http\n```\n\n官方文档列出的 HTTP 安全默认值包括：\n\n- 默认绑定 `127.0.0.1`。\n- `eval` 工具默认禁用。\n- 文件操作限制在 `OPENPENCIL_MCP_ROOT`。\n- CORS 默认禁用。\n- 可配置 `OPENPENCIL_MCP_AUTH_TOKEN`。\n- 默认端口为 `7600`，可用 `PORT` 覆盖。\n\n## 典型工作流\n\n1. 打开 OpenPencil app 并加载 `.fig` 文档，或创建新文档。\n2. 启动/配置 MCP server。\n3. 在 Claude Code 或其他 Agent 中确认 MCP 连接。\n4. 先读取页面树、节点、选区或页面列表。\n5. 根据目标创建或修改 shape、layout、fill、stroke、effects。\n6. 需要时重新组织节点、分组、复制或删除。\n7. 保存 `.fig` 文件。\n\n## 常用工具方向\n\n- 文档：`open_file`、`save_file`、`new_document`。\n- 读取：`get_page_tree`、`find_nodes`、`get_node`、`list_pages`、`get_selection`。\n- 创建：`create_shape`、`render`。\n- 修改：`set_fill`、`set_stroke`、`set_layout`、`update_node`、`set_effects`。\n- 结构：`reparent_node`、`group_nodes`、`clone_node`、`delete_node`。\n\n## 提示词写法\n\n```text\n使用 open-pencil MCP 读取当前页面结构，找出 hero section 的文字和按钮节点，并给出可落地的前端实现建议。\n```\n\n```text\n使用 open-pencil MCP 在当前画布创建一个小型 SaaS dashboard hero 区块，创建前先读取现有页面尺寸和字体。\n```\n\n## 风险和注意事项\n\n- MCP 工具可以修改设计文件，保存前要确认目标文件和变更意图。\n- 不要对未知目录开放宽泛的 `OPENPENCIL_MCP_ROOT`。\n- 自动允许 `mcp__open-pencil__*` 比全局 bypass 权限更窄，但仍应只在可信项目使用。\n- 设计文件可能包含未公开产品信息，使用前确认数据边界。\n- 如果只需要只读分析，提示词中明确“只读取，不修改，不保存”。\n\n## 与其他工具的关系\n\n- 与 [[AI记录/内容/frontend-design|frontend-design]]：OpenPencil MCP 提供设计稿读写能力；frontend-design 提供前端界面设计判断。\n- 与 [[AI记录/内容/playwright-interactive|playwright-interactive]]：前者操作设计稿，后者验证浏览器里的实现。\n- 与 [[AI记录/内容/context7|context7]]：Context7 查框架文档；OpenPencil MCP 查设计结构。\n\n## 使用判断\n\n如果你需要 AI 直接读写 OpenPencil 设计文件，且能控制设计文件目录和权限，就可以使用。若只是不确定 `pencil` 指向哪个产品，应先核对名称再安装。\n\n## 来源\n\n- [OpenPencil MCP Server 文档](https://openpencil.dev/programmable/mcp-server)\n- [OpenPencil GitHub](https://github.com/open-pencil/open-pencil)"
    },
    {
      "id": "note-41",
    ...
    },
    {
      ...
      "summary": "Taste Skill Taste Skill 是面向 AI coding agents 的前端设计 Skill 集合，定位是减少 AI 生成界面的模板感和“AI 味”，让 agent 在布局、字体、动效、空间和视觉密度上有更明确的审美约束。 一句话定位 给 Codex、Claude Code、Cursor、Gemini CLI、v0、Lovable 等 a...",
      "wordCount": 868,
      "sourceFile": "mydata/我的数据/AI记录/内容/Taste-Skill.md",
      "markdown": "# Taste-Skill\n\nTaste-Skill 是面向 AI coding agents 的前端设计 Skill 集合，定位是减少 AI 生成界面的模板感和“AI 味”，让 agent 在布局、字体、动效、空间和视觉密度上有更明确的审美约束。\n\n## 一句话定位\n\n给 Codex、Claude Code、Cursor、Gemini CLI、v0、Lovable 等 agent 使用的前端审美约束包，适合把普通 AI UI 输出拉到更稳定、更有设计判断的水平。\n\n## 核心能力\n\n- 通过 `SKILL.md` 给 agent 注入前端设计规则。\n- 用参数控制设计倾向，而不是每次手写长 prompt。\n- 提供多种子 skill，例如 `taste-skill`、`gpt-tasteskill`、`redesign-skill`、`soft-skill`、`minimalist-skill`、`output-skill`、`image-to-code-skill` 等。\n- 用 anti-slop 规则禁止常见 AI 页面套路，例如默认字体、泛用卡片网格、空洞占位内容和未经定制的组件默认样式。\n\n## 三个核心旋钮\n\n官方文档把核心 `taste-skill` 描述为三旋钮系统，取值通常为 1 到 10：\n\n- `DESIGN_VARIANCE`：控制布局实验程度，低值更规整，高值更不对称、更现代。\n- `MOTION_INTENSITY`：控制动效强度，低值偏静态，高值会引入更强的滚动、磁吸或戏剧性动效。\n- `VISUAL_DENSITY`：控制信息密度，低值更宽松，高值更接近 dashboard 或 cockpit。\n\n## 适合场景\n\n- 生成产品官网、landing page、SaaS 页面、dashboard 和营销页。\n- 希望 AI 少用默认紫色渐变、三列卡片和模板化首屏。\n- 需要同一个项目里反复保持相近的视觉标准。\n- Codex 或 GPT 模型生成 UI 时，需要更强的布局变化和动效方向。\n- 已有页面需要 `redesign-skill` 做视觉审查和重构建议。\n\n## 不适合场景\n\n- 纯后端、CLI、数据处理任务。\n- 只需要严格复刻既有设计稿，不希望 agent 自行发挥。\n- 对无障碍、性能和产品可用性要求很高，但没有后续人工 review。\n- 业务界面要求安静、密集、重复操作时，不能直接套高动效和高视觉冲击参数。\n\n## 与其他设计 Skills 的区别\n\n- 与 [[AI记录/内容/frontend-design|frontend-design]]：`frontend-design` 更像一个单一官方前端设计工作流；Taste-Skill 更像一个可安装、可调参、带多个子 skill 的前端审美框架。\n- 与 [[AI记录/内容/UI Aesthetics|UI Aesthetics]]：UI Aesthetics 更强调“修正和精炼”；Taste-Skill 更强调从生成前就设置设计参数和禁用套路。\n- 与 [[AI记录/内容/ui-ux-pro-max|ui-ux-pro-max]]：UI UX Pro Max 更像综合 UI/UX 规则库；Taste-Skill 更聚焦 anti-slop 和视觉输出风格。\n\n## 使用方式\n\n官方文档给出的通用安装方式：\n\n```bash\nnpx skills add Leonxlnx/taste-skill\n```\n\nCodex 场景可使用：\n\n```bash\nnpx skills add Leonxlnx/taste-skill -a codex\n```\n\n安装前应先确认当前 agent 是否支持 `SKILL.md`，以及安装命令会写入哪个目录。\n\n## 风险和注意事项\n\n- 参数越高不等于越好，高动效和高视觉密度会增加实现复杂度。\n- Anti-slop 规则可能和既有品牌规范冲突，需要按项目调整。\n- 生成结果仍要检查响应式、可访问性、文字溢出、交互状态和性能。\n- 作为第三方开源 skill，使用前应检查仓库内容和许可证。\n\n## 使用判断\n\n如果你希望 AI 默认生成的前端页面更少模板感，并且愿意通过参数管理视觉倾向，Taste-Skill 值得作为常用设计 skill。若只是修已有页面的细节，优先考虑 [[AI记录/内容/UI Aesthetics|UI Aesthetics]] 或 [[AI记录/内容/impeccable|impeccable]]。\n\n## 来源\n\n- [Taste Skill 官网](https://www.tasteskill.dev/)\n- [Taste Skill Documentation](https://www.tasteskill.dev/docs)\n- [Leonxlnx/taste-skill GitHub](https://github.com/Leonxlnx/taste-skill)"
    },
    {
      "id": "note-56",
      "title": "trae",
    ...
    },
    {
      ...
      "summary": "UI Aesthetics 是一个面向多平台 AI agent 的 UI 审美增强 skill 包，用于生成、评审和重构更有视觉判断力的 Web UI。官方 README 将其定位为让界面更 refined、balanced、precise、product grade，同时避免盲目扩 scope 或堆砌“假高级感”。 一句话定位 当页面已经有基本功能或结构，...",
      "wordCount": 862,
      "sourceFile": "mydata/我的数据/AI记录/内容/UI Aesthetics.md",
      "markdown": "# UI Aesthetics\n\nUI Aesthetics 是一个面向多平台 AI agent 的 UI 审美增强 skill 包，用于生成、评审和重构更有视觉判断力的 Web UI。官方 README 将其定位为让界面更 refined、balanced、precise、product-grade，同时避免盲目扩 scope 或堆砌“假高级感”。\n\n## 一句话定位\n\n当页面已经有基本功能或结构，但需要更好的视觉层级、间距、字体、状态和产品质感时使用。\n\n## 核心能力\n\n- 生成 Web UI 时加入更清晰的审美判断。\n- Review 现有界面，指出为什么看起来像 AI 生成。\n- Refactor 设置面板、dashboard、表单、卡片、表格、overlay 等 UI。\n- 打磨 hover、focus、selected、loading、error 和 dark mode 状态。\n- 约束阴影、模糊、高光、层级和动效，让它们服务信息，而不是表演。\n\n## 工作模式\n\n官方仓库把 skill 优化为六类工作模式：\n\n1. Generation\n2. Review\n3. Refactor\n4. Component Polish\n5. State / Motion Refinement\n6. Depth / Lighting Refinement\n\n## 适合场景\n\n- “这个页面看起来太 AI 了，帮我改得更像产品。”\n- dashboard、设置页、后台、表格、表单等需要更精细但不想过度装饰。\n- 需要保持信息密度，不想把页面改成大卡片营销页。\n- 想检查默认状态、交互状态、暗色模式和深度逻辑。\n- 多 agent 或多 IDE 中希望复用同一套 UI polish 规则。\n\n## 不适合场景\n\n- 纯后端、CLI、数据库、算法任务。\n- 从零做强品牌首屏并追求极端视觉冲击，这类更适合 [[AI记录/内容/frontend-design|frontend-design]] 或 [[AI记录/内容/Taste-Skill|Taste-Skill]]。\n- 已有严格设计稿，需要逐像素复刻时，应以设计稿和视觉回归为准。\n- 用户明确只要功能实现，不要求 UI 打磨。\n\n## 支持平台\n\n仓库提供多个平台入口，包括：\n\n- Codex：`.codex/skills/ui-aesthetics/`\n- Claude Code：`.claude/skills/ui-aesthetics/`\n- Cursor：`.cursor/rules/ui-aesthetics.mdc`\n- OpenCode、Windsurf、Gemini CLI、Qoder、Kiro、GitHub Copilot 等。\n\n不同平台的触发方式不同，常见形式包括 `$ui-aesthetics`、`/ui-aesthetics`、`@ui-aesthetics` 或 prompt file 选择。\n\n## 核心原则\n\n- 保持用户要求的范围，不自行扩张功能。\n- 先修层级，再做装饰。\n- 克制优先于假高级感。\n- 默认状态保持安静，语义状态保持明确。\n- 动效和深度用于解释结构，而不是制造噪音。\n- 输出应可交付、可维护。\n\n## 与其他设计 Skills 的区别\n\n- 与 [[AI记录/内容/frontend-design|frontend-design]]：`frontend-design` 更偏从需求直接创造有强视觉方向的界面；UI Aesthetics 更偏审美修正、状态细化和产品级 polish。\n- 与 [[AI记录/内容/Taste-Skill|Taste-Skill]]：Taste-Skill 有参数化的生成前约束；UI Aesthetics 更适合在生成、评审、重构和组件打磨之间切换。\n- 与 [[AI记录/内容/impeccable|impeccable]]：两者都能做 polish；UI Aesthetics 更聚焦 Web UI 审美和组件状态，Impeccable 更像综合质量与表达打磨。\n\n## 风险和注意事项\n\n- 审美建议仍需要结合产品语境，不能替代设计负责人判断。\n- 视觉打磨可能改变信息优先级，修改前要确认关键业务动作不被弱化。\n- 使用多平台 wrapper 时，应以仓库根目录 `SKILL.md` 作为 canonical source。\n- 对真实产品仍需要浏览器检查：响应式、文字溢出、对比度、键盘焦点和状态变化。\n\n## 使用判断\n\n如果目标是让现有 UI 更干净、更稳、更像真实产品，而不是创造一个强风格 demo，UI Aesthetics 是很合适的默认选择。\n\n## 来源\n\n- [kasonye/ui-aesthetics-skill GitHub](https://github.com/kasonye/ui-aesthetics-skill)"
    },
    {
      "id": "note-58",
      "title": "ui-ux-pro-max",
